...
pub fn parse_header(file: &[u8]) -> Result<Header> {
    if file.len() < HEADER_LEN + 12 {
        return Err(Error::Format("file too small".into()));
    }
    if &file[0..4] != MAGIC {
        return Err(Error::Format("bad magic, not a DSSS save".into()));
    }
...
pub fn decrypt(file: &[u8], steamid: u64) -> Result<Vec<u8>> {
    let header = parse_header(file)?;
    if header.flags & FLAG_MANDARIN == 0 {
        return Err(Error::Format(format!(
            "flags {:#x} are not Mandarin",
            header.flags
        )));
    }
    Ok(mandarin::decrypt(payload(file), decrypted_len(file), steamid)?)
}
...
pub fn crack_with<F: Fn(u64) + Sync>(file: &[u8], report: F) -> Option<u64> {
    mandarin::crack_with(
        payload(file),
        decrypted_len(file),
        mandarin::STEAMID_BASE,
