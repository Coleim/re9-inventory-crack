...
pub fn decrypt(encrypted: &[u8], decrypted_len: u64, key: u64) -> Result<Vec<u8>, String> {
    ...
    println!("encrypted_data.len() = {}", encrypted.len());
    println!("decrypted_data_length = {}", decrypted_len);
...
pub fn crack(encrypted: &[u8], decrypted_len: u64, base: u64, count: u64) -> Option<u64> {
    ...
    println!("initial_state: {:016x}", initial_state);
    ...
    (0..num_chunks)
        ...
        .find_map_any(|ci| {
            ...
            eprint!("\r  cracked {done} / {count} candidates");
            found
        })
        .map(|id| {
            eprintln!();
...
mod test {
    ...
    #[test]
    fn test_decrypt() {
        let payload = fs::read("data011Slot.bin")
            .expect("failed to read data011Slot.bin");
