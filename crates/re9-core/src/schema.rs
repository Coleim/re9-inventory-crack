...
fn load() -> HashMap<u32, HashSet<u32>> {
    ...
    let text = include_str!("../re9_fields.tsv");
...
fn load_struct_types() -> HashMap<u64, String> {
    let mut map = HashMap::new();
    let text = include_str!("../re9_structs.tsv");
    for line in text.lines() {
        let mut it = line.splitn(2, '\t');
...
fn load_enums() -> HashMap<String, HashMap<i64, String>> {
    let mut map = HashMap::new();
    let text = include_str!("../re9_enums.tsv");
    for line in text.lines() {
        let mut it = line.splitn(2, '\t');
...
/// Struct type hint (e.g. `via.vec3`) for a given class hash + field hash.
...
/// The set of valid field hashes for a given class hash, used to detect and
/// resync from parser desyncs.
