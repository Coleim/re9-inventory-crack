...
use crate::mandarin::{plan_block::plan_block, splitmix::splitmix};
...
pub fn decrypt(encrypted_data: &[u8], steamid: u64, decrypted_data_length: u64) -> Vec<u8> {
    ...
    println!("decrypted_data_length = {}", decrypted_data_length);
    ...
    println!("nb blocks: {}", blocks.len());
    println!("blocks: {:?}", blocks);
    println!("sum(blocks) = {}", blocks.iter().sum::<u64>());
    println!(
        "bank_len*nb_blocks + sum(blocks) = {}",
        bank_len as u64 * blocks.len() as u64 + blocks.iter().sum::<u64>()
    );
    println!("encrypted_data.len() = {}", encrypted_data.len());
    println!("decrypted_data_length = {}", decrypted_data_length);
    ...
    for bloc_idx in 0..blocks.len() {
        ...
        // --- DEBUG ---
        println!(
            "[decrypt] block {bloc_idx}: bank_start_enc={bank_start_enc}, data_start_enc={data_start_enc}, data_end_enc={data_end_enc}, block_size={block_size}, encrypted_data.len()={}",
            encrypted_data.len()
        );
        if data_end_enc > encrypted_data.len() {
            println!(
                "[decrypt] >>> OVERFLOW at block {bloc_idx}: need {} bytes, have {} (deficit = {})",
                data_end_enc,
                encrypted_data.len(),
                data_end_enc as i64 - encrypted_data.len() as i64
            );
            panic!("block {bloc_idx}: out of bounds in encrypted_data");
        }
        // --- END DEBUG ---
        //
        //
